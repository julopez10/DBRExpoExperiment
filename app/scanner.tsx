...
export default function Scanner(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    let receiver = cvr.addResultReceiver({
      onProcessedDocumentResultReceived: (result) => {
        if (
          ...
          (ifBtnClick.current ||
            result.detectedQuadResultItems[0].crossVerificationStatus ===
              EnumCrossVerificationStatus.CVS_PASSED)
        ) {
    ...
    cvr
      .startCapturing(EnumPresetTemplate.PT_DETECT_DOCUMENT_BOUNDARIES)
      .catch(/*no-op*/);
  ...
  }, [cvr, camera, cameraView, ifBtnClick, router]);
